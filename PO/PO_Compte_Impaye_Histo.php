...
//traitement
    for ($m = 1; $m != $inter + 1; $m++) {
        ...
        $sql = "SELECT * FROM impaye WHERE date_impaye BETWEEN STR_TO_DATE('$yearmin-$monthmin-$day $hour:$min:$sec', '%Y-%m-%d %H:%i:%s') AND STR_TO_DATE('$year-$month-$day $hour:$min:$sec', '%Y-%m-%d %H:%i:%s') AND num_siren='$numsiren' ORDER BY date_impaye;";
        $req = $cnx->query($sql);
        $montant = 0;
